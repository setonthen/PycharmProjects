...
def test(decide,rep):
    for x in range(3):
        if decide[x][0] == decide[x][1] == decide[x][2]:
            print('The game ended, the winner is ' + decide[x][0])
            return 10
        elif decide[0][x] == decide[1][x] == decide[2][x]:
            print('The game ended, the winner is ' + decide[0][x])
            return 10
    if decide[2][0] == decide[1][1] == decide[0][2]:
        print('The game ended, the winner is ' + decide[2][0])
        return 10
    elif decide[1][1] == decide[2][2] == decide[0][0]:
