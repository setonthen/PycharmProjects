class Polynome:
    ...
    def make_polynomial(self):
        end_element=self.factor[len(self.factor)-1]
        if end_element==0:
            self.factor.pop()
            self.exponent.pop()
...
def process_addition(list1_fac, list1_exp, list2_fac, list2_exp):
    ...
    while index != len(list1_exp):
        for x in range(len(list2_exp)):
            ...
            elif list1_exp[len(list1_exp)-1] != list2_exp[x] and x==len(list2_exp)-1:
                flag1=False
        if flag1 == False:
        ...
        for x in range(len(list2_exp)):
            if list1_exp[index] == list2_exp[x]:
                ...
                if flag1 == True:
                    result = list1_fac[index] - list2_fac[x]
                    new_fac.append(result)
            elif list1_exp[len(list1_exp) - 1] != list2_exp[x] and x == len(
                    list2_exp) - 1:
                flag1 = False
        if flag1 == False:
            new_fac.append(list1_fac[index])
